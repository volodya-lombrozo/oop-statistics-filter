...
public class ReportNew {
    ...
    ReportNew(final String filename, final StatisticsCase... all) {
        this(Arrays.asList(all), filename);
    }

    private ReportNew(final Collection<StatisticsCase> all, final String filename) {
        this.cases = all;
    ...
    void make() throws IOException {
        ...
        this.output.print(ReportNew.headers(table));
        table.stream().map(ReportNew::values).forEach(this.output::print);
        this.output.close();
    }
